...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Validate HWND and platform window state. If invalid, free allocated viewport data and bail out
    // rather than triggering an assertion (useful during early/platform init or when PlatformHandle
    // does not contain a real HWND). This prevents CreateSwapChainForHwnd from being called with
    // an invalid HWND which returns DXGI_ERROR_INVALID_CALL.
    if (hwnd == 0 || !viewport->PlatformWindowCreated || !::IsWindow(hwnd))
    {
        // Cleanup allocated viewport data to avoid leaks.
...
{
    if (vd == nullptr || vd->CommandQueue == nullptr)
        return;
...
{
    if (vd == nullptr || vd->SwapChainWaitableObject == nullptr)
        return nullptr;
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*)
{
    ImGui_ImplDX12_Data* bd = ImGui_ImplDX12_GetBackendData();
    ImGui_ImplDX12_ViewportData* vd = (ImGui_ImplDX12_ViewportData*)viewport->RendererUserData;
    if (vd == nullptr || vd->SwapChain == nullptr || vd->CommandList == nullptr || vd->SwapChainWaitableObject == nullptr)
        return;

    ImGui_ImplDX12_FrameContext* frame_context = ImGui_WaitForNextFrameContext(vd);
    if (frame_context == nullptr || frame_context->CommandAllocator == nullptr)
        return;

    UINT back_buffer_idx = vd->SwapChain->GetCurrentBackBufferIndex();
    if (vd->FrameCtx[back_buffer_idx].RenderTarget == nullptr)
        return;
