...
#pragma endregion
	...
	while (msg.message != WM_QUIT) {
		...
		if (winApp->ProcessMessage()) {
			...
			ImGui::DragFloat3("sprite pos", &sprite->GetTransform().translate.x, 0.3f);
			ImGui::ColorEdit4("sprite color", &sprite->GetMaterial()->color.x, 1.0f); // クリアカラーの編集
			ImGui::DragFloat2("UV translate", &sprite->GetUVTransform().translate.x, 0.01f, -10.0f, 10.0f);
			ImGui::DragFloat2("UV scale", &sprite->GetUVTransform().scale.x, 0.01f, 0.0f, 10.0f);
			ImGui::SliderAngle("UV rotate", &sprite->GetUVTransform().rotate.z);
