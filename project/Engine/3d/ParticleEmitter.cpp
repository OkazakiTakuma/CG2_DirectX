...
#include "ParticleManager.h" // Manager邵ｺ・ｮ陞ｳ螟ゑｽｾ・ｩ邵ｺ謔滂ｽｿ繝ｻ・ｦ繝ｻ
#include "algorithm.h"
#include <algorithm>
#include <fstream>
#include <cmath>
#include <cstdlib>
#include <iomanip> // std::setw騾包ｽｨ繝ｻ繝ｻSON郢ｧ蝣､・ｶ・ｺ魄怜干竊楢ｬｾ・ｹ髯ｦ蠕鯉ｼ邵ｺ・ｦ闖ｫ譎擾ｽｭ蛟･笘・ｹｧ荵昶螺郢ｧ繝ｻ・ｼ繝ｻ
...
// 陟大｢鍋・邵ｺ・ｧ陷ｿ蜉ｱ・陷ｿ謔ｶ笆ｲ邵ｺ貅ｷﾂ・､郢ｧ蛛ｵﾎ鍋ｹ晢ｽｳ郢昜ｻ呻ｽ､逕ｻ辟夂ｸｺ・ｫ隴厄ｽｸ邵ｺ蟠趣ｽｾ・ｼ郢ｧﾂ
ParticleEmitter::ParticleEmitter() : groupName_(""), count_(0), frequency_(0.0f), frequencyTimer_(0.0f), textureFilePath_(""), isActive_(true), blendMode_(kBlendModeNormal) {
	...
	// 隨渉隨渉隨渉 邵ｺ蜷ｶ竏狗ｸｺ・ｦ邵ｺ・ｮ郢昜ｻ｣ﾎ帷ｹ晢ｽ｡郢晢ｽｼ郢ｧ・ｿ邵ｺ・ｫ陞ｳ迚吶・邵ｺ・ｪ陋ｻ譎・ｄ陋滂ｽ､郢ｧ雋槭・郢ｧ蠕娯ｻ邵ｺ鄙ｫ・･ 隨渉隨渉隨渉
	...
	emitParam_.lifeTime = 1.0f; // 陝・ｽｿ陷ｻ・ｽ邵ｺ繝ｻ邵ｺ・ｰ邵ｺ・ｨ闕ｳﾂ霑ｸ・ｬ邵ｺ・ｧ雎ｸ蛹ｻ竏ｴ邵ｺ・ｦ邵ｺ蜉ｱ竏ｪ邵ｺ繝ｻ竏ｪ邵ｺ繝ｻ
...
void ParticleEmitter::Update(float deltaTime) {
	// 郢ｧ・｢郢ｧ・ｯ郢昴・縺・ｹ晄じ縲堤ｸｺ・ｯ邵ｺ・ｪ邵ｺ繝ｻ・ｼ蛹ｻ縺檎ｹ晁ｼ披・髫ｪ・ｭ陞ｳ螢ｹ・・ｹｧ蠕娯ｻ邵ｺ繝ｻ・九・迚呻｣ｰ・ｴ陷ｷ蛹ｻ繝ｻ隴厄ｽｴ隴・ｽｰ郢晢ｽｻ隰ｾ・ｾ陷・ｽｺ郢ｧ螳夲ｽ｡蠕鯉ｽ冗ｸｺ・ｪ邵ｺ繝ｻ
	if (!isActive_) {
		return;
	}
	...
	// 鬯・ｽｻ陟趣ｽｦ邵ｺ・ｫ陟｢諛環ｧ邵ｺ貅ｯ繝ｻ陷榊｢灘鋤陷・ｽｺ陷・ｽｦ騾・・
	frequencyTimer_ += deltaTime;
	if (frequency_ > 0.0f && frequencyTimer_ >= frequency_) {
		Emit();
		// 郢ｧ・ｿ郢ｧ・､郢晄ｧｭ繝ｻ郢ｧ蛛ｵﾎ懃ｹｧ・ｻ郢昴・繝ｨ繝ｻ驛・ｽｶ繝ｻ邃・崕繝ｻ・定題ｼ費ｿ･邵ｺ阮吮・邵ｺ・ｧ雎・ｽ｣驕抵ｽｺ邵ｺ・ｪ鬮｢鬥ｴ蝗育ｹｧ蜑・ｽｿ譏ｴ笆ｽ繝ｻ繝ｻ
...
// =========================================================
// JSON邵ｺ・ｸ邵ｺ・ｮ闖ｫ譎擾ｽｭ莨懊・騾・・
// =========================================================
void ParticleEmitter::SaveToJson(const std::string& filePath) {
	nlohmann::json root;

	// 隴鯉ｽ｢陝・･繝ｻ郢晁ｼ斐＜郢ｧ・､郢晢ｽｫ邵ｺ蠕娯旺郢ｧ蠕後・闕ｳﾂ陟趣ｽｦ髫ｱ・ｭ邵ｺ・ｿ髴趣ｽｼ郢ｧ阮吶堤ｸｺ荵晢ｽ芽叉鬆大ｶ檎ｸｺ髦ｪ笘・ｹｧ蜈ｷ・ｼ莠包ｽｻ謔ｶ繝ｻ郢ｧ・ｰ郢晢ｽｫ郢晢ｽｼ郢晏干繝ｻ郢昴・繝ｻ郢ｧ・ｿ郢ｧ蜻茨ｽｶ蛹ｻ・・ｸｺ・ｪ邵ｺ繝ｻ笳・ｹｧ繝ｻ・ｼ繝ｻ
	...
	// 邵ｺ阮吶・郢ｧ・ｰ郢晢ｽｫ郢晢ｽｼ郢晄圜・ｼ莠包ｽｾ蜈ｷ・ｼ繝ｻFire"邵ｺ・ｪ邵ｺ・ｩ繝ｻ蟲ｨ繝ｻ郢昴・繝ｻ郢ｧ・ｿ郢ｧ蜑・ｽｽ諛医・
	...
	groupJson["blendMode"] = static_cast<int>(blendMode_); // 隨倥・・ｿ・ｽ陷会｣ｰ繝ｻ螢ｹ繝ｶ郢晢ｽｬ郢晢ｽｳ郢晏ｳｨﾎ皮ｹ晢ｽｼ郢晏ｳｨ・定将譎擾ｽｭ繝ｻ
	groupJson["meshType"] = static_cast<int>(meshType_);

	// emitParam繝ｻ蝓滂ｽｧ遏ｩﾂ・ｰ闖ｴ阮吶・闕ｳ・ｭ髴・ｽｫ繝ｻ蟲ｨ・定ｭｬ・ｼ驍上・
	nlohmann::json param;
	...
	// 郢晢ｽｫ郢晢ｽｼ郢晏現竊鍋ｹｧ・ｰ郢晢ｽｫ郢晢ｽｼ郢晄憺倹邵ｺ・ｧ郢ｧ・ｻ郢昴・繝ｨ
	root[groupName_] = groupJson;

	// 郢晁ｼ斐＜郢ｧ・､郢晢ｽｫ邵ｺ・ｫ隴厄ｽｸ邵ｺ蟠趣ｽｾ・ｼ邵ｺ・ｿ繝ｻ蛹ｻ縺・ｹ晢ｽｳ郢昴・ﾎｦ郢昜ｺ･・ｹ繝ｻ邵ｺ・ｧ驍ｯ・ｺ魄怜干竊鍋ｹ晁ｼ斐°郢晢ｽｼ郢晄ｧｭ繝｣郢晁肩・ｼ繝ｻ
...
// =========================================================
// JSON邵ｺ荵晢ｽ臥ｸｺ・ｮ髫ｱ・ｭ邵ｺ・ｿ髴趣ｽｼ邵ｺ・ｿ陷・ｽｦ騾・・
// =========================================================
void ParticleEmitter::LoadFromJson(const std::string& filePath) {
	// 隨渉隨渉隨渉 隨倥・・ｪ・ｭ邵ｺ・ｿ髴趣ｽｼ郢ｧﾂ陷鷹亂竊鍋ｸｲ竏壺穐邵ｺ螢ｼ繝ｻ邵ｺ・ｦ邵ｺ・ｮ郢昜ｻ｣ﾎ帷ｹ晢ｽ｡郢晢ｽｼ郢ｧ・ｿ郢ｧ雋橸ｽｮ迚吶・邵ｺ・ｪ郢昴・繝ｵ郢ｧ・ｩ郢晢ｽｫ郢昜ｺ･ﾂ・､邵ｺ・ｫ郢晢ｽｪ郢ｧ・ｻ郢昴・繝ｨ邵ｺ蜷ｶ・・隨渉隨渉隨渉
	// 繝ｻ繝ｻSON陷繝ｻ竊馴ｬ・・蟯ｼ邵ｺ謔滂ｽｭ莨懈Β邵ｺ蜉ｱ竊醍ｸｺ繝ｻ・ｰ・ｴ陷ｷ蛹ｻ繝ｻ邵ｲ竏夲ｼ・ｸｺ阮吶帝坎・ｭ陞ｳ螢ｹ・邵ｺ貅ｷﾂ・､邵ｺ蠕娯落邵ｺ・ｮ邵ｺ・ｾ邵ｺ・ｾ闖ｴ・ｿ郢ｧ荳奇ｽ檎ｸｺ・ｾ邵ｺ蜻ｻ・ｼ繝ｻ
	count_ = 0;
	frequency_ = 0.0f;
	textureFilePath_ = "";
	isActive_ = true;
	...
	// 隨渉隨渉隨渉 邵ｺ阮呻ｼ・ｸｺ荵晢ｽ雨SON郢晁ｼ斐＜郢ｧ・､郢晢ｽｫ邵ｺ・ｮ髫ｱ・ｭ邵ｺ・ｿ髴趣ｽｼ邵ｺ・ｿ 隨渉隨渉隨渉
	std::ifstream ifs(filePath);
	if (!ifs.is_open()) {
		return; // 郢晁ｼ斐＜郢ｧ・､郢晢ｽｫ邵ｺ蠕娯・邵ｺ莉｣・檎ｸｺ・ｰ郢昴・繝ｵ郢ｧ・ｩ郢晢ｽｫ郢昜ｺ･ﾂ・､邵ｺ・ｮ邵ｺ・ｾ邵ｺ・ｾ驍ｨ繧・ｽｺ繝ｻ
	}
	...
	// 髢ｾ・ｪ陋ｻ繝ｻ繝ｻ郢ｧ・ｰ郢晢ｽｫ郢晢ｽｼ郢晄憺倹邵ｺ郢ｰSON邵ｺ・ｮ闕ｳ・ｭ邵ｺ・ｫ陝・ｼ懈Β邵ｺ蜷ｶ・狗ｸｺ荵昴Γ郢ｧ・ｧ郢昴・縺・
	if (!root.contains(groupName_)) {
		return; // 隴・ｽｰ邵ｺ蜉ｱ・･闖ｴ諛岩夢邵ｺ貅倥Τ郢晢ｽｼ郢昴・縺・ｹｧ・ｯ郢晢ｽｫ邵ｺ・ｪ邵ｺ・ｩ邵ｺ・ｧJSON邵ｺ・ｫ陝・ｼ懈Β邵ｺ蜉ｱ竊醍ｸｺ繝ｻ・ｰ・ｴ陷ｷ蛹ｻ・らｹ昴・繝ｵ郢ｧ・ｩ郢晢ｽｫ郢昜ｺ･ﾂ・､邵ｺ・ｧ驍ｨ繧・ｽｺ繝ｻ
	}

	auto& groupJson = root[groupName_];

	// JSON邵ｺ・ｮ闕ｳ・ｭ邵ｺ・ｫ鬯・・蟯ｼ邵ｺ蠕娯旺郢ｧ蜿･・ｰ・ｴ陷ｷ蛹ｻ繝ｻ邵ｺ・ｿ邵ｲ竏壹Ι郢晁ｼ斐°郢晢ｽｫ郢昜ｺ･ﾂ・､郢ｧ蜑・ｽｸ鬆大ｶ檎ｸｺ髦ｪ笘・ｹｧ繝ｻ
	...
	// emitParam邵ｺ・ｮ闕ｳ・ｭ髴・ｽｫ郢ｧ螳夲ｽｪ・ｭ邵ｺ・ｿ髴趣ｽｼ郢ｧﾂ
